...
export default function SearchHeader({ childId, query }: SearchHeaderProps) {
	return (
		<header className="border-b border-violet-100 bg-white/80 px-6 py-5 backdrop-blur-md">
			<div className="mx-auto flex max-w-[1450px] flex-col gap-5 lg:flex-row lg:items-center">
				<Link href={`/child-dashboard?childId=${childId}`} className="shrink-0">
					<Image
				...
				>
					<ArrowLeft size={19} aria-hidden="true" className="text-violet-600" />
					Retour à l&apos;accueil
				</Link>
			</div>
		</header>
